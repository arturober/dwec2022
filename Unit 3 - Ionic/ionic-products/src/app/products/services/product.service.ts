...
export class ProductService {
  ...
  getProducts(): Observable<Product[]> {
    return this.http.get<{ products: Product[] }>(this.BASE_URL).pipe(
      map((resp) =>
        resp.products.map((p) => {
  ...
  getProduct(id: number): Observable<Product> {
    return this.http.get<{ product: Product }>(`${this.BASE_URL}/${id}`).pipe(
      map((resp) => {
  ...
  changeRating(id: number, rating: number): Observable<number> {
    return this.http
      .put<{ rating: number }>(`${this.BASE_URL}/${id}/rating`, { rating })
      .pipe(map((resp) => resp.rating));
  }

  addProduct(prod: Product): Observable<Product> {
    return this.http.post<{ product: Product }>(this.BASE_URL, prod).pipe(
      map((resp) => {
        const p = resp.product;
        p.imageUrl = environment.baseUrl + '/' + p.imageUrl;
        return p;
  ...
  deleteProduct(idProd: number): Observable<void> {
    return this.http.delete<void>(`${this.BASE_URL}/${idProd}`);
  }
